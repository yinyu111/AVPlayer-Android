...
public class YYMP4Muxer {
    public static final int YYMuxerErrorCreate = -2200;
    public static final int YYMuxerErrorAudioAddTrack = -2201;
    public static final int YYMuxerErrorVideoAddTrack = -2202;

    private static final String TAG = "YYMuxer";
    private YYMuxerConfig mConfig = null;///< 封装配置
    private YYMuxerListener mListener = null;///< 回调
    ...
    private List<YYBufferFrame> mVideoList = new ArrayList<>();///< 视频输入缓存

    private int mAudioTrackIndex = -1;///< 音频track轨道下标
    private MediaFormat mAudioFormat = null;///< 音频输入视频格式描述
    private List<YYBufferFrame> mAudioList = new ArrayList<>();///< 音频输入缓存

    private boolean mIsStart = false;
    private Handler mMainHandler = new Handler(Looper.getMainLooper());///< 主线程


    public YYMP4Muxer(YYMuxerConfig config, YYMuxerListener listener) {
        mConfig = config;
        mListener = listener;
    }
    ...
    private boolean _hasAudioTrack() {
        return (mConfig.muxerType.value() & YYMediaBase.YYMediaType.YYMediaAudio.value()) != 0;
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    private boolean _hasVideoTrack() {
        return (mConfig.muxerType.value() & YYMediaBase.YYMediaType.YYMediaVideo.value()) != 0;
    }
    ...
    private void _setupMuxer(){
        ...
        if(mMediaMuxer == null){
            try {
                ...
                _callBackError(YYMuxerErrorCreate,e.getMessage());
                return;
            }
        }
    }
    ...
    private void _setupVideoTrack() {
        ...
        if(mVideoFormat != null){
            ...
            try {
                ...
                _callBackError(YYMuxerErrorVideoAddTrack,e.getMessage());
            }
        }
    }
    ...
    private void _setupAudioTrack() {
        ...
        if(mAudioFormat != null){
            ...
            try {
                ...
                Log.e(TAG, "addTrack" + e);
                _callBackError(YYMuxerErrorAudioAddTrack,e.getMessage());
            }
        }
    }
    ...
    private void _avInterleavedBuffers() {
        ...
        if(_hasVideoTrack() && _hasAudioTrack()){
            while (mAudioList.size() > 0 && mVideoList.size() > 0){
                YYBufferFrame audioPacket = mAudioList.get(0);
                YYBufferFrame videoPacket = mVideoList.get(0);
    ...
    private void _appendAudioBuffers() {
        ...
        while (mAudioList.size() > 0){
            YYBufferFrame packet = mAudioList.get(0);
